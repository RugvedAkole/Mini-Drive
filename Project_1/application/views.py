...
def browse_materials(request):
    ...
    used_storage = Material.objects.filter(uploaded_by=request.user).aggregate(total=Sum('file_size'))['total'] or 0
    storage_limit = request.user.studentprofile.storage_limit

    context = {
        'materials': materials,
        'used_storage': used_storage,
        'storage_limit': storage_limit,
        'used_percentage': min(100, int((used_storage / storage_limit) * 100)) if storage_limit else 0,
...
def upload_material(request):
    ...
    if request.method == 'POST':
        ...
        if used_storage + file_size > storage_limit:
            messages.error(request, f"Storage limit exceeded. You need {(used_storage + file_size - storage_limit) / (1024*1024):.2f} MB more space.")
